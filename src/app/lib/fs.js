...
async function dirHasChildren (dirPath) {
  try {
    const names = await fss.readdir(dirPath)
...
const fsExport = Object.assign(
  ...
  },
  {
    ...
    readlink: (p) => {
      return fss.readlink(p)
    },
    readdirWithStats: async (_path) => {
      if (_path === ROOT_PATH && isWin) {
        const names = await listWindowsRootPath()
