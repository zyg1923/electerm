/**
 * default setting
 */

export default {
  hotkey: 'Control+2',
  sshReadyTimeout: 50000,
  scrollback: 3000,
  onStartSessions: [],
  fontSize: 16,
  fontFamily: 'Maple Mono, mono, courier-new, courier, monospace',
  execWindows: 'System32/WindowsPowerShell/v1.0/powershell.exe',
  execMac: 'zsh',
  execLinux: 'bash',
  execWindowsArgs: [],
  execMacArgs: [],
  execLinuxArgs: [],
  enableGlobalProxy: false,
  disableConnectionHistory: false,
  disableTransferHistory: false,
  terminalBackgroundImagePath: '',
  terminalBackgroundFilterOpacity: 1,
  terminalBackgroundFilterBlur: 0,
  terminalBackgroundFilterBrightness: 1,
  terminalBackgroundFilterGrayscale: 0,
  terminalBackgroundFilterContrast: 1,
  terminalBackgroundText: '',
  terminalBackgroundTextSize: 48,
  terminalBackgroundTextColor: '#ffffff',
  terminalBackgroundTextFontFamily: 'Maple Mono',
  rendererType: 'dom',
  terminalType: 'xterm-256color',
  keepaliveCountMax: 10,
  keyword2FA: 'verification code,otp,one-time,two-factor,2fa,totp,authenticator,duo,yubikey,security code,mfa,passcode',
  saveTerminalLogToFile: false,
  checkUpdateOnStart: false,
  cursorBlink: false,
  cursorStyle: 'block',
  useSystemTitleBar: false,
  opacity: 1,
  defaultEditor: '',
  terminalWordSeparator: './\\()"\'-:,.;<>~!@#$%^&*|+=[]{}`~ ?',
  confirmBeforeExit: false,
  initDefaultTabOnStart: false,
  screenReaderMode: false,
  autoRefreshWhenSwitchToSftp: false,
  addTimeStampToTermLog: false,
  sftpPathFollowSsh: false,
  keepaliveInterval: 10000,
  backspaceMode: '^?',
  shiftEnterMode: '\\n',
  showHiddenFilesOnSftpStart: true,
  terminalInfos: [
    'uptime',
    'cpu',
    'mem',
    'activities',
    'network',
    'disks'
  ],
  remoteMonitorBarEnabled: false,
  remoteMonitorRefreshEnabled: true,
  remoteMonitorIntervalMs: 5000,
  opsStatusRefreshEnabled: false,
  opsStatusRefreshMs: 10000,
  remoteMonitorBarItems: [
    'hostname',
    'cpu',
    'cpuHistory',
    'memory',
    'upload',
    'download',
    'uptime',
    'users',
    'disks'
  ],
  filePropsEnabled: [
    'name',
    'size',
    'modifyTime'
  ],
  hideIP: false,
  dataSyncSelected: 'all',
  nameAI: '',
  baseURLAI: 'https://api.atlascloud.ai/v1',
  modelAI: 'deepseek-chat',
  roleAI: '终端专家,提供不同系统下命令,简要解释用法,用markdown格式',
  apiPathAI: '/chat/completions',
  proxyAI: '',
  sessionLogPath: '',
  transferTempDir: '',
  sshSftpSplitView: true,
  showCmdSuggestions: true,
  autoReconnectTerminal: false,
  restoreTerminalSessionOnReload: false,
  startDirectoryLocal: '',
  allowMultiInstance: false,
  disableDeveloperTool: false,
  dragDropBehavior: 'ask',
  switchTabOnHover: false,
  disableTabIndex: false,
  disableShortcutBar: false,
  disableConfirmForLargeClipboardContent: false,
  confirmOnMultilinePaste: true,
  clearMarkEnabled: true,
  opsApprovalTimeoutMinutes: 30,
  opsCollectConcurrency: 3,
  opsViIntercept: true,
  opsApprovalWebhook: '',
  leftSideBarIcons: [
    'newBookmark',
    'quickConnect',
    'bookmarks',
    'transferHistory',
    'opsCenter',
    'terminalThemes',
    'setting',
    'settingSync',
    'widgets'
  ]
}
