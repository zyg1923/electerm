...
import { uniqueOpenTabTitle, resolveHistoryTitle } from '../common/unique-tab-title.js'
...
export default Store => {
  ...
  Store.prototype.closeTabsLeft = function (id) {
    const { store } = window
    const { tabs } = store
    const targetTab = tabs.find(t => t.id === id)
    if (!targetTab) {
  ...
  Store.prototype.duplicateTab = function (tabId) {
    ...
    if (duplicatedTab.title) {
      const sessionTitle = uniqueOpenTabTitle(duplicatedTab.title, tabs)
      if (sessionTitle && sessionTitle !== duplicatedTab.title) {
        duplicatedTab.sessionTitle = sessionTitle
      } else {
        delete duplicatedTab.sessionTitle
      }
    }
  ...
  Store.prototype.addTab = function (
    ...
    if (newTab.title) {
      const sessionTitle = uniqueOpenTabTitle(newTab.title, tabs)
      if (sessionTitle && sessionTitle !== newTab.title) {
        newTab.sessionTitle = sessionTitle
      } else {
        delete newTab.sessionTitle
      }
    }
  ...
  Store.prototype.updateHistory = function (tab) {
    ...
    const { history } = store
    const histTitle = resolveHistoryTitle(tab) || tab.title
    const index = history.filter(d => d.id && d.tab).findIndex(d => {
      for (const key in tab) {
        if (tabPropertiesExcludes.includes(key) || key === 'title') {
          continue
        }
      ...
      return (resolveHistoryTitle(d.tab) || d.tab.title) === histTitle
    })
    if (index === -1) {
