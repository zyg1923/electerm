    ...
    try {
      ...
      setTimeout(
        () => {
          ...
          try {
            if (!window.localStorage.getItem('electerm-cmd-hints-v1')) {
              window.localStorage.setItem('electerm-cmd-hints-v1', '1')
              store.updateConfig({
                showCmdSuggestions: true
              })
            }
            if (!window.localStorage.getItem('electerm-sidebar-th-v1')) {
              window.localStorage.setItem('electerm-sidebar-th-v1', '1')
              const icons = Array.isArray(store.config.leftSideBarIcons)
                ? [...store.config.leftSideBarIcons]
                : []
              if (icons.length && !icons.includes('transferHistory')) {
                const idx = icons.indexOf('bookmarks')
                if (idx >= 0) {
