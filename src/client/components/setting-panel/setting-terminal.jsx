  ...
  render () {
    ...
    return (
      <div className='form-wrap pd1y pd2x'>
        ...
        <div className='pd2b'>
          <InputConfirm
            value={this.props.config.startDirectoryLocal}
            onChange={v => this.onChangeValue(v, 'startDirectoryLocal')}
            placeholder={startDirectoryLocalTxt}
            addonAfter={
              <Button
                onClick={async () => {
                  const path = await chooseSaveDirectory({
                    title: startDirectoryLocalTxt,
                    message: startDirectoryLocalTxt,
                    defaultPath: this.props.config.startDirectoryLocal || undefined
                  })
                  if (path) {
                    this.onChangeValue(path, 'startDirectoryLocal')
                  }
                }}
                type='text'
                size='small'
