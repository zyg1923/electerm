...
export default class SessionWrapper extends Component {
  ...
  renderTerminals = () => {
    ...
    const splitFill = this.isSplitViewEnabled()
    ...
    const pops = {
      ...
      width: splitFill ? '100%' : width,
      height: splitFill ? '100%' : height
    }
    return (
      <div
        className={cls}
  ...
  renderSftp = () => {
    ...
    const exts = {
      ...this.props,
      sftpPathFollowSsh,
      sshSftpSplitView: this.isSplitViewEnabled(),
      cwd,
      pid: id,
      enableSftp,
      sessionOptions,
  ...
  isSsh = () => {
    ...
    return !!(tab.authType || tab.host || tab.type === 'ssh')
  }
  ...
  renderViews = () => {
    ...
    const notSplitVew = !this.isSplitViewEnabled()
    ...
    const logH = this.props.tab.sessionLogFile ? 28 : 0
    splitterProps.style = {
      width: this.props.width + 'px',
      height: (this.props.height - logH) + 'px'
    }
    return (
  ...
  renderLogBar = () => {
    const file = this.props.tab.sessionLogFile
    if (!file) {
      return null
    }
    return (
      <div className='session-log-bar'>
        <b>{e('sessionLogEnabled')}</b>
        <span className='session-log-bar-path'>
          <ShowItem to={file}>{file}</ShowItem>
        </span>
      </div>
    )
  }
  ...
  render () {
    ...
    const cls = classnames(
      ...
      pane,
      {
        ...
        'ssh-sftp-split': this.isSplitViewEnabled()
      }
    )
    const divProps = {
      className: cls,
    ...
    return (
      ...
      >
        ...
        <SessionControl
          ...
          splitViewEnabled={this.isSplitViewEnabled()}
