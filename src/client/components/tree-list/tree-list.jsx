...
export default class ItemListTree extends Component {
  ...
  isBookmarkDoubleClick = (id, e) => {
    if (e.detail >= 2) {
      return true
    }
    return this._lastClickId === id &&
      Date.now() - (this._lastClickAt || 0) < 500
  }

  connectBookmarkById = (id) => {
    if (!id || this._connectingId === id) {
      return
    }
    this._connectingId = id
    this._lastClickId = ''
    this._lastClickAt = 0
    window.setTimeout(() => {
      if (this._connectingId === id) {
        this._connectingId = ''
      }
    }, 600)
    if (this.props.staticList) {
      const bookmark = this.props.bookmarks.find(d => d.id === id)
      if (bookmark && this.props.onClickItem) {
        this.props.onClickItem(bookmark)
      }
      return
    }
    window.store.onSelectBookmark(id)
  ...
  onDragStart = e => {
    if (Date.now() - (this._lastClickAt || 0) < 500) {
      e.preventDefault()
      return
    }
    let {
