...
import { resolveBookmarkSessionLog } from '../../../common/bookmark-session-log.js'
...
export const socketMixin = {
  ...
  async remoteInit (term = this.term) {
    ...
    const bookmarkLog = await resolveBookmarkSessionLog(tab)
    if (bookmarkLog) {
      opts.saveTerminalLogToFile = true
      opts.sessionLogPath = bookmarkLog.sessionLogPath
      opts.logName = bookmarkLog.logName
      this.props.tab.sessionLogFile = bookmarkLog.logFile
      this.setState({
        saveTerminalLogToFile: true,
        logPath: bookmarkLog.sessionLogPath,
        logFileName: bookmarkLog.logName + '.log'
      })
      notification.success({
        message: window.translate('sessionLogEnabled'),
        description: bookmarkLog.logFile,
        duration: 8
      })
    }
